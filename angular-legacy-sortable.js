...
wisbooSortable.directive('wisbooSortable', ['$parse', 'wisbooSortableConfig', function ($parse, config) {
  ...
  return {
    ...
    link: function link(scope, $el) {
      var el = $el[0];
      var options = angular.extend(scope.wisbooSortable || {}, config);
      var watchers = [];
      var offDestroy;
      var sortable;
      el[expando$1] = scope.collection;
      ...
      evt) {
        ...
        if (el !== evt.from) {
          var prevItems = evt.from[expando$1];
          removed = prevItems[oldIndex];
          prevItems.splice(oldIndex, 1);
          items.splice(newIndex, 0, removed);
          evt.from.insertBefore(evt.item, nextSibling); // revert element
        } else {
          items.splice(newIndex, 0, items.splice(oldIndex, 1)[0]); // move ng-repeat comment node to right position
      ...
      function _destroy() {
        offDestroy();
        angular.forEach(watchers, function (
        /** Function */
        unwatch) {
          unwatch();
        });
